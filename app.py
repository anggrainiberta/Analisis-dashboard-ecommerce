...
with tab2:
    ...
    daily = (
        df_master.groupby(df_master['transaction_time'].dt.date)
...
with tab3:
    ...
    with col_a:
        ...
        for c in sorted(cust['cluster'].unique()):
            ...
            ax.scatter(sub['total_transaction_pelanggan'], sub['total_spent_pelanggan'],
    ...
    if 'age_group' in cust.columns:
        ...
        with col_d:
            ...
            age_tx = cust.groupby('age_group')['total_transaction_pelanggan'].sum().sort_values()
            fig, ax = plt.subplots(figsize=(6, 4))
...
with tab4:
    ...
    cat = category_df.loc[fitur_cat.index].copy()
    ...
    with col_a:
        ...
        if 'category' in cat.columns:
            top_cat = cat.groupby('category')['total_revenue_kategori'].sum().sort_values().tail(10)
            fig, ax = plt.subplots(figsize=(6, 4))
    ...
    with col_b:
        ...
        for c in sorted(cat['cluster'].unique()):
            ...
            ax.scatter(sub['banyak_transaksi_categori'], sub['total_revenue_kategori'],
                       label=f"Cluster {c}", alpha=0.6)
        ax.set_xlabel("Jumlah Transaksi"); ax.set_ylabel("Total Revenue")
        ax.legend()
        st.pyplot(fig); plt.close(fig)

    if 'product_name' in cat.columns:
        st.subheader("Top 10 Produk berdasarkan Revenue")
        top_prod = cat.groupby('product_name')['total_revenue_kategori'].sum().sort_values(ascending=False).head(10)
    ...
    summary2 = cat.groupby('cluster')[['total_revenue_kategori', 'banyak_transaksi_categori']].mean().round(2)
